...
public class MenuPrincipalController {
    ...
    private void afficherActivitesRobots() {
        Map<String, List<Activite>> activites = ActiviteUtil.lireActivitesDepuisFichier("Application\\src\\main\\java\\Donnees\\Activites.json");
        Map<String, List<Robot>> robots = RobotUtil.lireRobotsDepuisFichier("Application\\src\\main\\java\\Donnees\\Robots.json");
    ...
    if (activites != null && activites.containsKey(compte.getCourriel())) {
        ...
        while (!retour) {
            ...
            switch (choixModif) {
                ...
                case 2:
                    try {
                        activite.setDateDebut(LocalDate.parse(view.getDetail("Entrez la nouvelle date de début (YYYY-MM-DD)"), DateTimeFormatter.ISO_LOCAL_DATE));
                ...
                case 3:
                    try {
                        activite.setDateFin(LocalDate.parse(view.getDetail("Entrez la nouvelle date de fin (YYYY-MM-DD)"), DateTimeFormatter.ISO_LOCAL_DATE));
                    } catch (DateTimeParseException e) {
                        System.out.println("Format de date invalide. Veuillez réessayer.");
                    }
                    break;
...
private void supprimerActivite() {
        ...
        while (!retour) {
            ...
            switch (choix) {
                ...
                case 3:
                    ...
                    break;
                case 4:
                    ((Fournisseur) compte).setCapaciteFabrication(Integer.parseInt(view.getDetail("Entrez la nouvelle capacité de fabrication")));
                    break;
                case 5:
                    retour = true;
                    break;
    ...
    private void afficherEtatRobots() {
        Map<String, List<Robot>> robotsInventaires = RobotUtil.lireRobotsDepuisFichier("Application\\src\\main\\java\\Donnees\\Robots.json");
        ...
        if (robotsUtilisateur != null && !robotsUtilisateur.isEmpty()) {
            ...
            for (int i = 0; i < robotsUtilisateur.size(); i++) {
                ...
                System.out.println((i + 1) + ". Nom : " + robot.getNom() + ", Type : " + robot.getType() + ", Niveau de batterie : " + robot.getNiveauBatterie());
            }
    
            int choix = Integer.parseInt(view.getDetail("Entrez le numéro du robot pour voir les détails complets ou 0 pour revenir"));
    ...
    Map<String, List<Composante>> inventaires = ComposanteUtil.lireComposantesDepuisFichier("Application\\src\\main\\java\\Donnees\\Composantes.json");
    ...
    if (composantesUtilisateur != null) {
        ...
        if (autresComposantes.isEmpty()) {
            System.out.println("Erreur : Vous devez avoir au moins une autre composante pour enregistrer un robot.");
            return;
        }
        ...
        Robot nouveauRobot = new Robot(cpuSelectionne, nom, type, position, vitesse, niveauBatterie, consoCPU, memoire);

        System.out.println("Sélectionnez les autres composantes pour le robot :");
        for (int i = 0; i < autresComposantes.size(); i++) {
            System.out.println((i + 1) + ". " + autresComposantes.get(i).getNom() + " (" + autresComposantes.get(i).getType() + ")");
        }
        ...
        ComposanteUtil.ecrireComposantesDansFichier(inventaires, "Application\\src\\main\\java\\Donnees\\Composantes.json");
    ...
    private void enregistrerComposante() {
        ...
        String typeStr = view.getDetail("Entrez le type de la composante (CPU, ROUES, BRAS, HELICE, HAUTPARLEUR, MICRO, ECRANTXT, ECRANGUI)");
            ...
            while (!retour) {
                ...
                switch (choixModif) {
                    ...
                    case 3:
                        ...
                        break;
                    case 4:
                        composante.setType(ComposanteType.valueOf(view.getDetail("Entrez le nouveau type (CPU, ROUES, BRAS, HELICE, HAUTPARLEUR, MICRO, ECRANTXT, ECRANGUI)")));
                        break;
                    case 5:
                        composante.setIsDisponible(Boolean.parseBoolean(view.getDetail("La composante est-elle disponible ? (true/false)")));
                        break;
                    case 6:
                        retour = true;
                        break;
    ...
    private void listeComposantesDisponibles() {
        ...
        try {
            JsonNode rootNode = objectMapper.readTree(new File("Application\\src\\main\\java\\Donnees\\Composantes.json"));
            ...
            if (inventairesNode != null) {
                ...
                if (!composantesDisponibles.isEmpty()) {
                    int choix = Integer.parseInt(view.getDetail("Entrez le numéro de la composante pour voir les détails ou 0 pour revenir"));
        ...
        for (Composante composante : composantesFournisseur) {
            if (composante.getNom().equals(nomComposante) && composante.getIsDisponible()) {
                ...
                if (composante instanceof CPU) {
                    ...
                    composantesUtilisateur.add(new CPU(cpu.getNom(), cpu.getDescription(), cpu.getPrix(), false, cpu.getNumeroDeSerie()));
                } else {
    ...
    private void trouverFournisseur() {
        ...
        if (inventaires != null) {
            ...
            while (!retour) {
                ...
                int choix = Integer.parseInt(view.getDetail("Entrez le numéro du fournisseur pour voir ses informations ou " + (fournisseurs.size() + 1) + " pour revenir au menu principal"));
